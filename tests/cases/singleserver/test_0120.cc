...
int main() {
  ...
  ret = getxattr(TEST_DATA_01, name, value, sizeof(value));
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN1, ret);

  ret = getxattr(TEST_DATA_01"_aaa", name, value, sizeof(value));
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN2, ret);

  ret = getxattr(TEST_DATA_02, name, value, sizeof(value));
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN3, ret);

  ret = getxattr(TEST_DATA_02"_bbb", name, value, sizeof(value));
