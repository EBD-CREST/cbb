...
#define TEST_NAME           "Truncate"
#define TEST_CASE           "01-11"
...
int main() {
  ...
  ret = truncate(TEST2_DATA_01, 100);
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN1, ret);
  ret = truncate(TEST2_DATA_01a, 100);
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN1, ret);

  ret = truncate(TEST2_DATA_01"_aaa", 100);
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN2, ret);
  ret = truncate(TEST2_DATA_01a"_aaa", 100);
  output_test_results(TEST_NAME, TEST_CASE, TEST_CASE_PATTERN2, ret);
